...
import com.example.test.network.HttpTaskEventArg;
import com.example.test.network.HttpTaskListener;
import com.example.test.network.HttpTaskMgr;
...
public class DataRequest extends AsyncTask<Object, Object, Object> 
	...
	protected Object doInBackground(Object... params) {
		...
		if (mTryCache) {
			...
			if (cache != null) {
				DataCenter.Response resp = cache.loadCacheResponse(context, mDataId);
				if (resp != null) {
	...
	private void notifyResponseHandler(int dataId, DataCenter.Response resp) {
		if (mRespHandler != null) {
			mRespHandler.obtainMessage(MSG_DATA_OBTAINED, dataId, 0, resp).sendToTarget();
		}
	}
	
	private DataCenter.Response processResponse(byte[] dataBytes) {
		try {
			...
			DataCenter.Response resp = processor.processData(mDataId, dataBytes);
...
//			return "http://192.168.91.201:9101/cfappstore/v2/upgrade";
	...
	private static class RespHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			if (callback != null) {
				DataCenter.Response resp = null;
				if (msg.obj == null || !(msg.obj instanceof DataCenter.Response)) {
					resp = new DataCenter.Response();
					resp.mStatusCode = DataCenter.Response.STATUS_FAILED;
					resp.mSuccess = false;
				} else {
					resp = (DataCenter.Response) msg.obj;
				}
