...
//		Log.e("demo", "writer count = "+ mFileWriterMgr.getWriterCount());
	...
	private void notifyProgress() {
		...
		if (mTaskListener != null) {
			...
			{
				ArrayList<FileWriterMgr.FileWriter> array = mFileWriterMgr.getFileWriterList(m_taskid);
				if(array != null) {
					mArg.mThreadDownloadTaskList.clear();
					for(FileWriterMgr.FileWriter writer : array) {
						DownloadTask.ThreadDownloadTask threadTask = new DownloadTask.ThreadDownloadTask();
	...
	public void continueMultiDowloader(DownloadTask task) {
		for(DownloadTask.ThreadDownloadTask threadTask : task.mThreadDownloadTaskList) {
